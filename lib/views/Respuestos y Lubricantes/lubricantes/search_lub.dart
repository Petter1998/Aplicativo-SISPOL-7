...
class SearchLubricantesView extends StatefulWidget {
  final List<Lubricante> searchResults;

  const SearchLubricantesView({super.key, required this.searchResults});

  @override
  // ignore: library_private_types_in_public_api
  _SearchLubricantesViewState createState() => _SearchLubricantesViewState();
}

class _SearchLubricantesViewState extends State<SearchLubricantesView> {
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
          ...
          pw.TableHelper.fromTextArray(
            headers: <String>[
              'ID', 'ID Emisor', 'Nombre', 'Fecha de Ingreso', 'Fecha de Vencimiento', 'Capacidad', 'Marca', 'Proveedor', 'Precio', 'Stock', 'Tipo', 'Viscosidad'],
            data: widget.searchResults.map((lubricante) => [
              lubricante.id.toString(),
              lubricante.idUser.toString(),
              lubricante.nombre,
              dateFormat.format(lubricante.fechaIngreso),
              dateFormat.format(lubricante.fechaVence),
              lubricante.capacidad.toString(),
              lubricante.marca,
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: DataTable(
          ...
          rows: widget.searchResults.map((lubricante) {
            return DataRow(cells: [
              _buildCell(lubricante.id.toString()),
              _buildCell(lubricante.idUser.toString()),
              _buildCell(lubricante.nombre),
              _buildCell(dateFormat.format(lubricante.fechaIngreso)),
              _buildCell(dateFormat.format(lubricante.fechaVence)),
        ...
        children: [
          ...
          FloatingActionButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => const LubricantesView()),
              );
            },
            backgroundColor: const Color.fromRGBO(56, 171, 171, 1),
